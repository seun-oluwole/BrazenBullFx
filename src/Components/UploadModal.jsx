...
export default function UploadModal() {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      const { data, error: uploadError } = await supabase.storage.from("profile-images").upload(filePath, file);

      if (uploadError) throw uploadError;

      const { data: url } = supabase.storage.from("profile-images").getPublicUrl(filePath);
  ...
  const handleRemoveImage = async () => {
    ...
    try {
      ...
      if (imageurl[0]?.image_url) {
        ...
        const { data, error: deleteError } = await supabase.storage.from("profile-images").remove([fileName]);

        if (deleteError) throw deleteError;

        const { error: updateError } = await supabase.from("wallet").update({ image_url: null }).eq("user_id", userId);
  ...
  return (
    <CustomModal isOpen={isUploadModalOpen} onClose={closeModal}>
      <div className={styles.mainContainer}>
        <span className={styles.xContainer} onClick={closeModal}>
          <HiOutlineXCircle className={styles.xIcon} />
        </span>
