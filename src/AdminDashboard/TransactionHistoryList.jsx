...
export default function TransactionHistoryList({ userId }) {
  const { allTransactions, setAllTransactions, fetchingInvestorTrans, fetchingTransError, fetchInvestorTransDetail } =
    useAdmin();
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "transactions" }, (payload) => {
        ...
        if (payload.new.user_id === userId) {
          if (payload.eventType === "INSERT") {
            setAllTransactions((prev) => [...prev, payload.new]);
          } else if (payload.eventType === "UPDATE") {
            setAllTransactions((prev) =>
              prev.map((item) => (item.id === payload.new.id ? { ...item, ...payload.new } : item))
            );
          } else if (payload.eventType === "DELETE") {
            setAllTransactions((prev) => prev.filter((item) => item.id !== payload.old.id));
          }
        }
      })
      .subscribe();
  ...
  if (fetchingInvestorTrans)
    return (
      <div className={styles.spinnerContainer}>
        <LoadingSvg width={30} height={30} />
      </div>
    );
  if (allTransactions.length <= 0 && fetchingTransError)
    return <div className={styles.error}>Check your connection and try again.</div>;
  return (
    <>
                  ...
                  <tr className={styles.bodyRow} key={index} onClick={() => handleTransactionDetail(id)}>
                    ...
                    <td className={styles.bodyCell}>
                      <div className={styles.amountContainer}>
                        <div className={styles.amount}>{`${getCurrencySymbol(
                          balance_currency
                        )}${converted_amount?.toLocaleString()}`}</div>
